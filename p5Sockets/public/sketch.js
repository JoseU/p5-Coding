...
function draw() {
  ...
  //console.log('Sending: ' + centerX + " " + centerY);
  ...
  lerpColor(colors[0], colors[2], mouseX/width),
...
function newDrawing(data) {
  //console.log('data de x' + ' ' + data.x);


  if (data.mouY >= height / 2) {
    alpha = map(data.mouY, height / 2, height, 149, 5);
    strokeWeight(map(data.mouY, height / 2, height, 0.2, 1.61880 * 1.25));

  } else {
    alpha = map(data.mouY, 0, height, 5, 149);
    strokeWeight(map(data.mouY, height / 2, height, 1.61880 * 1.25, 0.2))
  };
  ...
  // floating towards mouse position
  data.centX += (data.mouX - data.centX) * 0.01;
  data.centY += (data.mouY - data.centY) * 0.01;
